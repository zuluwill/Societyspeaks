...
A stack that bottoms out in `ph.shutdown()` / `client.join` is the PostHog hang. `worker_exit` no longer calls those APIs (drain by `qsize`, pause consumers). The SDK still registers `atexit.register(self.join)` with no timeout — recycle then hangs the main greenlet after `worker_exit` (PYTHON-FLASK-JD, 2026-08-24). We unregister that handler and no-op `join` under gevent. Recurring `WORKER TIMEOUT` / `worker_abort` whose dump is **not** in `posthog.client.join` is a real request stall and should still page — do not add those phrases to the lifecycle drop list. SIGTERM / SIGKILL / “Perhaps out of memory?” (gunicorn’s generic abort text, not proven OOM) stay dropped.
