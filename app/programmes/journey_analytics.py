"""Guided-journey analytics: topic rankings + PostHog lifecycle captures.

`compute_topic_rankings` powers `flask journey-theme-analytics` (curriculum
planning). The PostHog helpers below capture flow-critical events (started /
step_completed / completed) on vote POSTs; recap GET is an idempotent backup
for completed only, and only when this visitor actually finished — sitemap
crawlers must not count.
"""
...
from app.models import Discussion, Programme, Statement, StatementVote, TrendingTopic
