...
def _consensus_worker_heartbeat_ok():
    ...
    clustering run would block — see app.lib.process_heartbeat).
...
def init_scheduler(app):
    ...
    @scheduler.scheduled_job('cron', minute=0, id='send_journey_reminders', max_instances=1, coalesce=True, misfire_grace_time=3600)
    def send_journey_reminders_job():
        """
        Send journey reminder emails to subscribers whose next_send_at is due.

        Runs every hour at :00 so that timezone-aware scheduling reaches users at
        their chosen local time regardless of UTC offset.  The query is cheap —
        next_send_at is indexed and only returns rows that are actually due.
